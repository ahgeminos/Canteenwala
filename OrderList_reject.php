...
<body>
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $sql1 = "INSERT INTO `sales`(`Order_id`, `food_id`, `combo_id`, `status`, `user_id`, `time`, `cost`) VALUES ('" . $row['Order_id'] . "','" . $row['food_id'] . "','" . $row['combo_id'] . "','" . $row['status'] . "','" . $row['user_id'] . "','" . $row['time'] . "','" . $row['cost'] . "')";
            ...
            <div class="odetails">
                ...
                if (!empty($food['food'])) {
                    foreach ($food['food'] as $item => $itemQuantity) {
                        $result1 = mysqli_query($conn, "select * from menu where `food_id`='" . $item . "';");
