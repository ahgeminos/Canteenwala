      ...
      if (!empty($_SESSION['cart'])) {
        foreach ($_SESSION['cart'] as $item => $itemquatity) {
          if ($itemquatity > 0) {
            if (preg_match('/^C.*/', $item)) {
              if (!empty($combo)) {
                $combo['combo'][$item] += ($itemquatity);
              } else {
                $combo['combo'][$item] = ($itemquatity);
              }
            } else {
              if (array_key_exists($item, $food)) {
                $food['food'][$item] += ($itemquatity);
              } else {
                $food['food'][$item] = ($itemquatity);
              }
            }
       ...
       $sql = "INSERT INTO `order_today`(`Order_id`, `food_id`, `combo_id`, `status`, `user_id`,`cost`) VALUES ('" . $id . "','" . $fid . "','" . $cid . "',0,'" . $uid . "','".$total."');";
        $result = mysqli_query($conn, $sql);
        $_SESSION['cart'] = array();

        echo '<script> window.alert("Your Order has been placed!");window.location.href="Userhome.php";</script>';
      } else {
        echo '<script>alert("Add items to the cart")</script>';
      }
    }
    ?>
